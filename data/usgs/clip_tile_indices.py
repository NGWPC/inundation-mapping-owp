...
def clip_3m_to_1m(
    ...
    three_meter_gdf: gpd.GeoDataFrame,
    verbose: bool = False
) -> gpd.GeoDataFrame:
    """
    ...
    if verbose: print("   One meter unary union...")
    one_meter_unary_union = one_meter_gdf.union_all()
    if verbose: print("   Removing 3m tiles that are covered by 1m tiles...")
...
def clip_tiles_to_wbd(
    ...
    if verbose: print("   Intersecting tile index with WBD...")
...
def main(
    ...
    if os.path.exists(one_meter_output_path):
        if overwrite:
            ...
            raise FileExistsError(f"Output tile index {one_meter_output_path} already exists. Use -o to overwrite.")
    if os.path.exists(three_meter_output_path):
        if overwrite:
            os.remove(three_meter_output_path)
        else:
            raise FileExistsError(f"Output tile index {three_meter_output_path} already exists. Use -o to overwrite.")
    ...
    assert one_meter_gdf.crs == three_meter_gdf.crs == wbd_gdf.crs, "CRS's of tile indices and WBD must be the same"
    ...
    if verbose: print("Getting union of all geometries in WBD...")
