...
class HucList:
    ...
    def _get_child_hucs_parallel(self, huc_level: int, verbose: bool, n_jobs: int) -> Dict[str, Set[str]]:
        ...
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc=f'Getting child HUC{huc_level}s with {n_jobs} workers', disable=(not verbose)):
    ...
    def _get_parent_hucs_parallel(self, huc_level: int, verbose: bool, n_jobs: int) -> Dict[str, Set[str]]:
        ...
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc=f'Getting parent HUC{huc_level}s with {n_jobs} workers', disable=(not verbose)):
                key, value = future.result()
                hucs[key] = value
    ...
    def _get_any_hucs_parallel(self, huc_level: int, verbose: bool, n_jobs: int) -> Dict[str, Set[str]]:
        ...
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            ...
            for future in tqdm(as_completed(futures), total=len(futures), desc=f'Getting HUC{huc_level}s with {n_jobs} workers', disable=(not verbose)):
                key, value = future.result()
                hucs[key] = value
